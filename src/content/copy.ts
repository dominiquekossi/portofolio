...
export const copy = {
  ...
  hero: {
    ...
    lede: {
      en: "Full-Stack Software Engineer building software end to end, Node and .NET on the backend, React and React Native powering motion-engineered interfaces on the front, all held to print-shop precision. I do my best work in fast-growing environments where the bar moves every week. That precision carries into the rest too: pixel-exact interface recreations used to train and evaluate AI models, AI-native tools of my own like an MCP orchestrator and a multi-provider LLM cost tracker, and instructions clear enough that a model, or a person, does exactly what you meant.",
      pt: "Engenheiro de Software Full-Stack, construo software de ponta a ponta: Node e .NET no backend, React e React Native construindo interfaces com motion no front, tudo com a mesma precisão de uma gráfica. Trabalho melhor em ambientes que crescem rápido, onde o nível sobe toda semana. Essa mesma precisão aparece no resto do trabalho também: recriações de interface pixel a pixel usadas para treinar e avaliar modelos de IA, ferramentas próprias e nativas de IA, como um orquestrador MCP e um rastreador de custo de LLM multi-provedor, além de instruções claras o bastante para que um modelo, ou uma pessoa, faça exatamente o que você quis dizer.",
    } as Bilingual,
